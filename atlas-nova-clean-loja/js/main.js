...
document.addEventListener('click', (e) => {
  ...
  if(inc) { changeQty(inc.dataset.inc, 1); refreshAll(); }
  if(dec) { changeQty(dec.dataset.dec, -1); refreshAll(); }
  if(rm) { removeItem(rm.dataset.rm); refreshAll(); }
  if(chip){
...
headerNav.addEventListener('click', (e) => {
  ...
  if(isNavLink && !isMegaTrigger && !isColCaret){
    ...
    headerNav.querySelectorAll('[aria-expanded="true"]').forEach(el => el.setAttribute('aria-expanded', 'false'));
  }
});
...
document.getElementById('copyright-line').textContent = `© ${new Date().getFullYear()} Atlas Nova Clean`;
