...
export function renderGrid(cat) {
  ...
  const types = PRODUCT_TYPES.filter((t) => t.cat === cat)
    .map((t) => {
      const variants =
        filterBrand === 'Todas' ? t.variants : t.variants.filter((v) => v.brand === filterBrand);
      return { ...t, variants };
    })
    ...
    .filter(
      (t) =>
        !searchTerm ||
        (t.name + ' ' + t.variants.map((v) => v.brand).join(' '))
          .toLowerCase()
          .includes(searchTerm),
    );

  document.getElementById('count-' + cat).textContent =
    types.length + (types.length === 1 ? ' produto' : ' produtos');
  ...
  const cardsHtml =
    visibleTypes
      .map((t) => (t.variants.length === 1 ? singleCard(t, t.variants[0]) : groupCard(t)))
      .join('') ||
    `<p style="color:var(--muted);font-size:14px;">Nenhum produto encontrado com esse filtro.</p>`;

  const showMoreHtml =
    types.length > VISIBLE_LIMIT
